...
struct digit * insertAtFront(struct digit * start, struct digit * new);
...
int countRedun(struct digit * ptr)
{
  struct digit * sortedArr=sortedCopy(ptr);
  ...
  while(sortedArr)
  {
    if (sortedArr->next)
    {
      if(sortedArr->num==sortedArr->next->num)
      {
        redundancies++;
      }
    }
    sortedArr=sortedArr->next;
  }
